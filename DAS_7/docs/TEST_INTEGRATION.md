...
Complete each level before moving to the next. If a higher-level test fails, check the newly
added interface first because the lower-level cluster has already passed.
...
The shared setup creates fixtures with unique identifiers:
...
Provider responses are controlled to make success and failure cases repeatable. The tests still
use the production implementations for database access, repositories, services, routing, and
authentication.

Teardown removes only the IDs registered during the current run. Database cascades remove the
related summaries, recommendations, preferences, links, and notification records. The
`afterAll` cleanup also runs after a failed assertion.

## 3. Test case specifications

Each specification uses the integration and system test-case format from the lecture slides.
