...
**Current state:** 14 suites, 130 passing cases. Two suites do not compile and are flagged at the bottom of this file; every other suite is green.

For the core features, [UNIT_TEST_CASES.md](UNIT_TEST_CASES.md) documents those same tests as a formal case table — precondition, input, expected output, expected postcondition. Sections below follow the same order as that table.

## Application models — track progress
`test/unit/track-progress/application-model.test.ts` — 9 cases
...
`test/unit/preferences/application-model.test.ts` — 4 cases
...
## HTTP — error mapping
`test/unit/http/error-mapper.test.ts` — 7 cases
...
`test/unit/infrastructure/llm-http-client.test.ts` — 20 cases
...
`test/unit/infrastructure/supabase-date-conversions.test.ts` — 18 cases
...
`test/unit/infrastructure/supabase-write-support.test.ts` — 15 cases
...
`test/unit/infrastructure/supabase-domain-mappers.test.ts` — 19 cases
...
`test/unit/infrastructure/supabase-domain-write-mappers.test.ts` — 14 cases
...
Both suites below are stale after the Supabase auth-model removal, which deleted `src/domain/entities/user.ts` and `src/domain/value-objects/account-type.ts` and added a required `studentId` to `EmailNotification`. Their cases are not counted above and are not in the case table.

### Domain — entities ⚠️
`test/unit/domain/entities.test.ts`
...
### Domain — value objects ⚠️
`test/unit/domain/value-objects.test.ts`
